...
void create_image(int global_index, double complex c) {
    char filename[256];
    sprintf(filename, "%s/mandelbrot_frame_%06d.png", OUT_DIR, global_index, creal(c));
...
int main(int argc, char **argv) {
    ...
    double start_real = -2.0;
    double end_real = 2.0;
    double step = 0.01;
    int total_images = (end_real - start_real) / step;

    int images_per_process = total_images / size;
    int remaining_images = total_images % size;

    double real_part = start_real + rank * images_per_process * step;
    ...
    for (int i=0;i<images_to_generate;i++) {
        double complex c = real_part + i * step;
